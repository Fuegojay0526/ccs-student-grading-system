...
--
-- Host: 127.0.0.1
-- Generation Time: Sep 18, 2022 at 01:41 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Program_Name_Request` (`_UserID` VARCHAR(100), `_FullName` VARCHAR(100), `_ProgramName` VARCHAR(100))   BEGIN
SELECT Record_ID, Users_Info, Time_Stamp FROM (SELECT Record_ID, Users_Info, Time_Stamp, UPPER(Users_Info) AS CapUserInfo FROM users_account) AS MyTable WHERE CapUserInfo->>'$.FULLNAME' = UPPER(_FullName) or CapUserInfo->>'$.STUDENTID' = UPPER(_UserID) and CapUserInfo->>'$.PROGRAMNAME' = UPPER(_ProgramName);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `Show_Grades` (`_UserID` VARCHAR(100), `_FullName` VARCHAR(100))   BEGIN
SELECT Record_ID, Users_Info, Time_Stamp FROM (SELECT Record_ID, Users_Info, Time_Stamp, UPPER(Users_Info) AS CapUserInfo FROM users_account) AS MyTable WHERE CapUserInfo->>'$.FULLNAME' = UPPER(_FullName) or CapUserInfo->>'$.STUDENTID' = UPPER(_UserID);
END$$
...
(1012, '{\"Name\": \"admin\", \"Status\": \"Active\", \"Password\": \"GjNh45lAqM3u-eb_ilY3EQ\", \"Username\": \"admin\", \"UserLevel\": \"Administrator\", \"EmailAddress\": \"jayfuego9@gmail.com\"}', 'C3x997F7253E5EF6911D5E746FCDD09A5D404EEE90E7F88AA71A536ED33A2C23', '2022-09-12 14:27:14');
...
--
ALTER TABLE `users_account`
  MODIFY `Record_ID` bigint NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1013;
COMMIT;
